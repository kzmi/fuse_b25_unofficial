	 ...
	 struct fuse_file_info *fi)
{
	...
	while (stream->err == 0 &&
		...
		pthread_cond_wait(&stream->buf_cond, &stream->buf_lock);
...
main(int argc, char **argv)
{
	...
	res = sscanf(mountpoint, "/dev/dvb/adapter%u/dvr0", &adapter);
	if (res != 1) {
	...
	if (res < 0 
		...
		syslog(LOG_NOTICE, "can't access the saved DVR0 device\n");
		return 1;
	}
